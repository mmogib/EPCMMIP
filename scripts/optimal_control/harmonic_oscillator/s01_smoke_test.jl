...
function build_algorithm(db, spec, method_name::AbstractString;
    ...
    if method_name == "AEFBFP"
        if aefbfp_preset === nothing
            return compressed_sensing_aefbfp_from_db(
                allow_untuned = allow_untuned_aefbfp,
                round_digits = aefbfp_round_digits,
            )
        end
...
function smoke_main()
    return benchmark_main(HARMONIC_OSCILLATOR_SPEC, [
        "--quick",
        "--force",
        "--initial-points=1",
        "--dims=100",
