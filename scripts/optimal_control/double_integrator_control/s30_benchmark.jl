# ============================================================================
# s30: Main benchmark for the double-integrator problem
# ============================================================================
#
# Goal:
#   Run the selected methods on the stored local double-integrator benchmark
#   dimensions / initial points and write the benchmark rows into the local DB.
#
# What this script reads:
#   - the local double-integrator problem definition
#   - the local benchmark configuration from ARGS
#   - the shared optimal-control AEFBFP winner only when AEFBFP is requested
#
# What this script writes:
#   - results/optimal_control/double_integrator_control/experiments.db
#   - results/optimal_control/double_integrator_control/logs/
#   - representative `history` rows for the reference dim/seed
#
# Simple workflow:
#   Before Section 1:
#     Load shared code, load the local problem definition, and define the
#     constants/helpers reused by the local s20 / s30 / s70 workflow.
#
#   Section 1:
#     Read benchmark options from ARGS or, in summary mode, load stored rows
#     from the local experiments database.
#
#   Section 2:
#     Open logging and print a short benchmark banner.
#
#   Section 3:
#     Run the benchmark loop: dim -> method -> initial point.
#
#   Section 4:
#     Reload the stored benchmark rows and print the final summary.
...
# ============================================================================
# Shared parsing and path helpers
# ============================================================================
...
# Local optimal-control outputs live under results/optimal_control/<problem>/...
...
# Main search loop: sample candidate params, run them on the local problem,
# score the candidates, then promote the best hash into tuned_winners.
...
function case_metric_summary(df, method::String, dim::Int)
    ...
    if nrow(solved) == 0
        return (nconv = nconv, ntot = ntot, iter = Inf, feval = Inf, cpu = Inf)
    end
    return (
        nconv = nconv,
        ntot = ntot,
...
function run_benchmark_cell(db, spec, cfg, run_id::AbstractString, prob::TestProblem,
                            dim::Int, method_name::String, init)
    alg = build_algorithm(db, spec, method_name;
                          allow_untuned_aefbfp = cfg.allow_untuned_aefbfp,
                          aefbfp_preset = cfg.aefbfp_preset,
                          aefbfp_round_digits = cfg.aefbfp_round_digits)
...
function print_benchmark_row(tee, method_name::String, dim::Int, init, cell)
    ...
    if cell.status == :error
        println(tee, "  $(method_name) K=$(dim) init=$(init.label) ERROR: $(cell.error_text)")
        return nothing
    end

    @printf(tee,
            "  %-8s K=%3d init=%-6s conv=%-5s iter=%5d fe=%5d nat=%10.3e\n",
            method_name, dim, init.label, string(cell.result.converged),
            cell.result.iterations, cell.result.f_evals, cell.native_residual)
    return nothing
end
...
function benchmark_main(spec, args = ARGS; script_name::AbstractString = "s30_benchmark")
    ...
    try
        ...
        # Section 4: final benchmark summary
        df = load_result_rows(db, spec; production = cfg.production, eps = cfg.eps, maxiter = cfg.maxiter)
...
function write_summary_table_tex(df, spec, path::String)
    ...
    open(path, "w") do io
        ...
        header_bottom = [" "]
        for _ in dims
            append!(header_bottom, ["Iter", "F-evals", "CPU"])
        end
        ...
        for method in methods
            ...
            for dim in dims
                ...
                iter_text = isfinite(stats.iter) ? string(Int(round(stats.iter))) : "DNC"
                fe_text = isfinite(stats.feval) ? string(Int(round(stats.feval))) : "DNC"
                cpu_text = isfinite(stats.cpu) ? @sprintf("%.3f", stats.cpu) : "DNC"
                ...
                append!(row, [iter_text, fe_text, cpu_text])
            end
            println(io, join(row, " & ") * " \\\\")
        end
...
function build_control_figure(rep, spec, figdir::String; png::Bool)
    ...
    plot!(plt, t, rep.init.x0;
          ...
          marker = :none)
    plot!(plt, t, rep.result.x;
          label = "AEFBFP computed",
    ...
    plot!(plt, t, exact_u;
          ...
          color = OC_CONTROL_STYLES.exact.color,
          lw = OC_CONTROL_STYLES.exact.lw,
          linestyle = OC_CONTROL_STYLES.exact.linestyle,
          marker = :none)
    plot!(plt; xlabel = "t", ylabel = "control", legend = :topleft)
    save_plot_files(plt, joinpath(figdir, "control_plot"); png = png)
end

function build_state_figure(rep, spec, figdir::String; png::Bool)
    prob = rep.prob
    comp = spec.state_trajectory(rep.result.x, prob)
    exact = exact_state(spec, prob)
    label1, label2 = spec.state_labels

    plt = single_panel_plot()
    plot!(plt, comp.t, comp.state1;
          label = "$(label1) computed",
    ...
    plot!(plt, comp.t, comp.state2;
          label = "$(label2) computed",
          color = OC_STATE_STYLES.comp2.color,
          lw = OC_STATE_STYLES.comp2.lw,
          linestyle = OC_STATE_STYLES.comp2.linestyle,
          marker = :none)
    plot!(plt, exact.t, exact.state1;
          label = "$(label1) exact",
          color = OC_STATE_STYLES.exact1.color,
          lw = OC_STATE_STYLES.exact1.lw,
          linestyle = OC_STATE_STYLES.exact1.linestyle,
          marker = :none)
    plot!(plt, exact.t, exact.state2;
          label = "$(label2) exact",
          color = OC_STATE_STYLES.exact2.color,
          lw = OC_STATE_STYLES.exact2.lw,
          linestyle = OC_STATE_STYLES.exact2.linestyle,
          marker = :none)
    plot!(plt; xlabel = "t", ylabel = "state", legend = :topleft)
    save_plot_files(plt, joinpath(figdir, "state_plot"); png = png)
end
...
function report_main(spec, args = ARGS)
    ...
    try
        ...
        hdf = load_history_rows(db, spec; production = cfg.production, eps = cfg.eps, maxiter = cfg.maxiter)
        if nrow(hdf) == 0
            println(tee, "No history rows found for $(spec.problem_name) at K=$(OC_REF_DIM), seed$(OC_REF_INIT).")
        else
            build_convergence_figure(hdf, spec, local_figdir(spec); png = cfg.png)
            println(tee, "wrote convergence_plot.pdf")
        end
        ...
        rep = solve_reference_run(db, spec;
                                  ...
                                  method_name = "AEFBFP",
                                  dim = OC_REF_DIM,
                                  seed_idx = OC_REF_INIT,
                                  allow_untuned_aefbfp = cfg.allow_untuned_aefbfp,
                                  aefbfp_preset = cfg.aefbfp_preset,
                                  aefbfp_round_digits = cfg.aefbfp_round_digits)
        build_control_figure(rep, spec, local_figdir(spec); png = cfg.png)
        build_state_figure(rep, spec, local_figdir(spec); png = cfg.png)
        println(tee, "wrote control_plot.pdf")
        println(tee, "wrote state_plot.pdf")
    finally
        teardown_logging(tee, logpath)
        SQLite.close(db)
    end
