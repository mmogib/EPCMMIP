# ============================================================================
# s70: Figures and tables for the double-integrator benchmark
# ============================================================================
#
# Goal:
#   Read the stored s30 benchmark rows for the local double-integrator problem
#   and turn them into the paper/report outputs.
#
...
#   - results/optimal_control/double_integrator_control/figures/tables.tex
#   - results/optimal_control/double_integrator_control/figures/di_convergence.pdf
#   - results/optimal_control/double_integrator_control/figures/di_control.pdf
#   - results/optimal_control/double_integrator_control/figures/di_state.pdf
...
function write_summary_table_tex(df, spec, path::String)
    ...
    open(path, "w") do io
        println(io, "% Auto-generated by optimal_control/double_integrator_control/s70_figures_tables.jl")
...
# These functions rebuild the representative convergence curve from a small
# re-run at the reference seed.
...
function build_control_figure(rep, spec, figdir::String; png::Bool,
    ...
    plot!(plt, t, rep.init.x0;
          ...
          marker = :none)
    for (j, (lo, hi)) in enumerate(((0.0, switch), (switch, spec.final_time)))
        idx = findall((lo .<= t) .& (t .< hi))
        level = median(rep.result.x[idx]) >= 0.0 ? 1.0 : -1.0
        plot!(plt, [lo, hi], [level, level];
    ...
    plot!(plt;
          ...
          ylims = (-1.1, 1.1),
          xticks = ([0.0, switch, spec.final_time], ["0", "1.2", "2"]),
          legend = :topleft)
    save_plot_files(plt, joinpath(figdir, stem); png = png)
end
...
function build_state_figure(rep, spec, figdir::String; png::Bool,
    ...
    plot!(plt, comp.t, comp.state2;
          ...
          marker = :none)
    ymax = max(maximum(comp.state1), maximum(comp.state2))
    plot!(plt; xlabel = L"t", ylabel = "State", legend = :topleft,
          xlims = (0.0, maximum(comp.t)),
          ylims = (0.0, max(ymax, 1.0e-12)))
    save_plot_files(plt, joinpath(figdir, stem); png = png)
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    report_main(DOUBLE_INTEGRATOR_SPEC)
end
