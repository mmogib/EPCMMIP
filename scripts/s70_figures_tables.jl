...
#      DNC marked. The MTTM/P1 α₁=0 artifact is excluded (footnoted).
...
const S70_METHOD_ORDER = ["EPCM", "EFBFP", "AEFBFP", "MTTM", "IMTTM", "IFRAB", "SFRBM"]
const S70_PROBLEMS     = ["P1", "P2", "P3", "P4"]
# Known artifacts excluded from tables/profiles. MTTM/P1: α₁=1 ⟹ first iterate 0,
# which on P1 (x*=0) "converges" in ~2 iters trivially — not a real result.
const ARTIFACT_EXCLUDE = Set([("MTTM", "P1")])
...
function write_latex_tables(df, tee)
    ...
    open(path, "w") do io
        ...
        println(io, "% MTTM/P1 excluded (α₁=1 artifact). DNC = no converged seed at native tol.")
