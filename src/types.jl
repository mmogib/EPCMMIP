...
#   - Symbol-keyed presets `:P1`–`:P4` (per-problem; EPCM's in `EPCM_PRESETS`,
#     baselines' in `configs/competitors_original_…`) and `:paper` (baselines'
#     source-paper defaults; used by `s28_validate_baselines.jl`). Fine-tuned
#     winners go to `configs/autogenerated_finetuned_…` (written by s20).
